...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case ID_FIRSTSPEED:
			...
			{
				showText(hwndEventLog, "Nie mo¿na zmieniac prêdkoci w czasie odtwarzania ruchu lub pracy automatycznej.");
			}
			break;
		case ID_SECONDSPEED:
			if (program_state == 0 || program_state == 3)
		...
		case ID_RECORD:
			...
			{
				showText(hwndEventLog, "Robot jest w trybie automatycznym. Zaczekaj a¿ skoñczy pracê lub wcinij RESET.");
			}
			else if (program_state == 4)
			{
				showText(hwndEventLog, "Robot odtwarza wczeniej nagrany ruch. Zaczekaj a¿ skoñczy pracê lub wcinij RESET.");
			}
			break;
		case ID_PLAY:
			if (program_state == 0 || program_state == 3)
		...
		case ID_AUTO:
			...
			{
				showText(hwndEventLog, "Robot odtwarza nagrany ruch. Zaczekaj a¿ skoñczy pracê lub wcinij RESET.");
			}
			break;
		case ID_RESET:
			resetEverything(hWnd);
		...
		case ID_BETA_MINUS:
			...
			{
				showText(hwndEventLog, "Zaczekaj a¿ robot skoñczy wykonywan¹ pracê lub wcinij RESET.");
			}
			break;
		case ID_ALPHA_MINUS:
			if (program_state == 0 || program_state == 3)
		...
		case ID_GRAB_LOSE:
			if (program_state == 0 || program_state == 3)
			{
				...
				else
				{
					...
					{
						SetWindowText(hwndGrabLose, L"PUÆ");
						attach_detachBox(hWnd);
						moveBox();
